     ...
     <div class="col-md-12 col-12 mb-5">
          <div class="card">
               ...
               <div class="card-header">
                    <button onclick="window.location.href='{{ route('admin.user.create') }}'" class="btn btn-success">Tạo thành viên</button>
               </div>
               <!-- table  -->
               <div class="card-body">
                              ...
                              <tbody>
                                   @if (isset($users) && is_object($users))
                                        @foreach ($users as $user)
                                             <tr>
                                                  <td>
                                                       <img width="30" src="https://ui-avatars.com/api/?background=random&name={{$user->fullName}}" alt="">
                                                  </td>
                                                  <td>{{ $user->fullName }}</td>
                                                  <td>{{ $user->email }}</td>
                                                  <td>{{ $user->phone }}</td>
